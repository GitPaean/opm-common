...
class Common
{
    ...
    // Scalar (float for float and for Evaluation<float, N> alike, see
    // Constants.hpp) and then converted, so that they come out bit-identical
    // to the quotients written directly in that type; Evaluation arithmetic
    // itself is not a constant expression under MSVC.
