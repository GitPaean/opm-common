...
void ESmry::read_ministeps_from_disk()
{
    ...
    // LF-separated record stream whose offsets below are byte counts, so
    // newline translation would desynchronise every seekg() from the file.
...
void ESmry::loadData(const std::vector<std::string>& vectList) const
{
    ...
    // Binary for both: see the note on the first fstream open above.
    fileH.open(dataFileList[dataFileIndex], std::ios::in | std::ios::binary);
