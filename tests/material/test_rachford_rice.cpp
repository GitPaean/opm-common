...
void
checkNearUnitRatios(const Scalar offset, const Scalar tolerance)
{
    const Dune::FieldVector<Scalar, 3> K {1 + offset, 1 + 3 * offset, 1 - 2 * offset};

    // Aim for L = 0.5 before rounding the composition to Scalar.
    std::array<double, 3> pole {};
    for (std::size_t i = 0; i < pole.size(); ++i) {
        const double ratioLessOne = static_cast<double>(K[i]) - 1;
        pole[i] = ratioLessOne / (1 + 0.5 * ratioLessOne);
    }
    const auto z1 = static_cast<Scalar>((-0.2 * pole[0] - 0.8 * pole[2]) / (pole[1] - pole[2]));
    const Dune::FieldVector<Scalar, 3> z {
        static_cast<Scalar>(0.2), z1, static_cast<Scalar>(0.8) - z1};

    // Compute an independent reference in the vapour fraction, using the
    // actual rounded inputs. Their root can differ from 0.5.
    const auto residual = [&](long double vapour_fraction) {
        long double value = 0;
        for (std::size_t i = 0; i < K.size(); ++i) {
            const auto dK = static_cast<long double>(K[i]) - 1;
            value += static_cast<long double>(z[i]) * dK / (1 + vapour_fraction * dK);
        }
        return value;
    };
    long double lower = 0;
    long double upper = 1;
    BOOST_REQUIRE_GT(residual(lower), 0);
    BOOST_REQUIRE_LT(residual(upper), 0);
    ...
    const auto reference = static_cast<Scalar>(1 - (lower + upper) / 2);

    const auto L = Opm::RachfordRice::solve(K, z);

    BOOST_TEST_CONTEXT("ratios offset from one by " << offset)
    {
        BOOST_CHECK_GE(L, Scalar {0});
        BOOST_CHECK_LE(L, Scalar {1});
        BOOST_CHECK_SMALL(L - reference, tolerance);
    }
}
...
// A small residual does not bound the error in the phase fraction when the
// ratios approach one: g flattens and the pole bracket widens with it, so
// neither one keeps the iterate inside [0, 1] on its own.
BOOST_AUTO_TEST_CASE(FloatNearUnitRatios)
{
    for (const float offset : {1e-3f, 1e-4f, 1e-5f, 1e-6f}) {
        checkNearUnitRatios(offset, 2e-6f);
    }
}

// In double, round-off keeps Newton's step above tolerance near unit ratios,
// so the solves at the two smaller offsets have to finish with bisection.
BOOST_AUTO_TEST_CASE(DoubleNearUnitRatios)
{
    for (const double offset : {1e-4, 3e-6, 1e-6}) {
        checkNearUnitRatios(offset, 1e-9);
    }
}
...
// A signed zero is still a component that stays in the liquid.
BOOST_AUTO_TEST_CASE(NegativeZeroRatio)
{
    const Dune::FieldVector<double, 2> K {-0.0, 3.0};
    const Dune::FieldVector<double, 2> z {0.5, 0.5};

    BOOST_CHECK_CLOSE_FRACTION(Opm::RachfordRice::solve(K, z), 0.75, 1e-12);
}
...
BOOST_AUTO_TEST_CASE(NearCriticalSinglePhase)
{
    const auto justAboveOne = std::nextafter(1.0, 2.0);
    const Dune::FieldVector<double, 3> K {justAboveOne, justAboveOne, justAboveOne};
...
BOOST_AUTO_TEST_CASE(InvalidInput)
{
    ...
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(nanK, validZ), Opm::NumericalProblem);
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(negativeK, validZ), Opm::NumericalProblem);
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(validK, infiniteZ), Opm::NumericalProblem);
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(validK, negativeZ), Opm::NumericalProblem);
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(validK, zeroZ), Opm::NumericalProblem);
}

// PTFlash recovers from these rejections, so they must not reach the error
// log that flow counts in its end-of-run summary.
BOOST_AUTO_TEST_CASE(NumericalRejectionsAreNotLogged)
{
    const auto counter = std::make_shared<Opm::CounterLog>(Opm::Log::DefaultMessageTypes);
    Opm::OpmLog::addBackend("RachfordRiceCounter", counter);

    const Dune::FieldVector<double, 2> unitK {1.0, 1.0};
    const Dune::FieldVector<double, 2> infiniteK {std::numeric_limits<double>::infinity(), 0.5};
    const Dune::FieldVector<double, 2> z {0.4, 0.6};
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(unitK, z), Opm::NumericalProblem);
    BOOST_CHECK_THROW(Opm::RachfordRice::solve(infiniteK, z), Opm::NumericalProblem);
    BOOST_CHECK_EQUAL(counter->numMessages(Opm::Log::MessageType::Error), std::size_t {0});

    Opm::OpmLog::removeBackend("RachfordRiceCounter");
}
