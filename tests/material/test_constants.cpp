...
// The derived constants must equal the quotient written directly in the
// floating-point type behind Scalar, bit for bit, for float, double and an
// autodiff Evaluation on either - which must carry the value and no
// derivatives. hRed has no other use, so this is what instantiates it.
